...
#DZ: duplicate fucntion definitions below - commented out!
#https://falco.org/docs/rules/
#def map_finding_severity(priority):
...
def ecs_convert_falco_log_to_asff(entry):
    ...
    #  {'container_id': '49cf9f30acb79534dd2a7e15e62bee7975f5ba83c4276489bc2f5fc335fb0111', 'container_name': '/ecs-Falco-2-falco-f6f695b2d9d4faafeb01', 
    # 'ec2_instance_id': 'i-0279e51d31fa06571', 'ecs_cluster': 'awsome_ecs_cluster', 'ecs_task_arn': 'arn:aws:ecs:us-west-1:133776528597:task/awsome_ecs_cluster/6c4fe172c8a04610b99f5815804d2333', 
    # 'ecs_task_definition': 'Falco:2', 
    # 'log': '* Setting up /usr/src links from host', 'source': 'stdout'}
    ...
    try: 
       ...
       print("NB! ECS to ASFF got 'log' JSON: ", message)
       severity = map_finding_severity(message["priority"])
    except:
       print("PROBLEM: ECS to ASFF looks like there's no 'log' element or 'priority' inside it! ")
    ...
    #Initiualiza and start building ASFF finding array
    finding = {}
...
#    }
    ...
    try: 
       ...
       print ("LOOK: In get_eks_details RAW Data type of message, output_fields: ", type(message), message["output_fields"])
       fields = message["output_fields"]
       #print ("CHECK: In get_eks_details log output_fields Type, Values are: ", type(fields), fields)
       
    except:
       print("PROBLEM: get_eks_details - looks like there's no log element for 'output_fields'?")
...
def eks_convert_falco_log_to_asff(entry):
    ...
    #check new data type
    #print("Parsed LOG DATA type:", type(logEntry))
    
    print ("NB!!! CHECK: EKS to ASFF parsed log LOG DATA type, contents: ", type(logEntry), logEntry )
    print ("---------------------------------------------")
    
    try: 
       #output = entry["log"]["output"]
       #use previously defined logEntry
       #TODO uncomment output = logEntry["output"]
       output = str(logEntry["output"])
       
       #severity = map_finding_severity(entry["log"]["priority"])
       severity = map_finding_severity(logEntry["priority"])
       print(" EKS to ASFF got output, severity:   ", output, severity)
    except:
      print("PROBLEM: EKS to ASFF there's no 'output' or 'priority' entries in the Log above!")
    ...
    if instance is None:  
      ...
      instance_resource = create_ec2_instance_resource(instance)
      print("OK: EKS to ASFF got instance_resource: ", instance_resource)
    ...
    for container_resource in eks_resources:
        print (" ========> adding eks_resource: ", container_resource)
        resources.append(container_resource)
...
def lambda_handler(event, context):
    ...
    for entry in data['logEvents']:
        ...
        if "ecs_cluster" in message:
            # Add debug print
            print ("-------------------------------------------------------------")
            # print ("BEFORE calling ecs_convert_falco_log_to_asff LOG Data type: ", type(message))
            # print ("-------------------------------------------------------------")
            # end debug print
        ...
        #DZ: added condition for expected EKS cluster if "ec2_instance_id"
            ...
            # Add debug print
            print ("-------------------------------------------------------------")
            # print ("BEFORE calling eks_convert_falco_log_to_asff LOG entry Data type: ", type(message))
            # print ("-------------------------------------------------------------")
            # end debug print
            finding = eks_convert_falco_log_to_asff(message)
        else:
            print ("===> Unknown CW Log event message detected - DONT KNOW HOW TO PROCESS!")

        #after getting security findings, append it to an array
        print ("AFTER calling XXXX_convert_falco_log_to_asff ASFF finding is: ", finding)
        print ("-------------------------------------------------------------")
    ...
    print ("YAY! Imported above findings into SecurityHub: ",r)
    return

#DZ new function to parse out in "log" field JSON substring to remove esscaped '\"'
def parseJSON(source): 

   #"log": "2023-02-02T00:43:56.734263328Z stdout F {\"hostname\":\"falco-z95gm\",\"output\":\"21:17:02.642052704: Error File below /etc opened for writing (user=<NA> user_loginuid=-1 command=touch /etc/26 pid=22583 parent=bash pcmdline=bash file=/etc/26 program=touch gparent=<NA> ggparent=<NA> gggparent=<NA> container_id=f687a1640776 image=docker.io/library/nginx) k8s.ns=default k8s.pod=nginx-test2 container=f687a1640776\",
   # \"priority\":\"Error\",\"rule\":\"Write below etc\",\"source\":\"syscall\",\"tags\":[\"filesystem\",\"mitre_persistence\"],\"time\":\"2023-02-01T21:17:02.642052704Z\", \"output_fields\": {\"container.id\":\"f687a1640776\",\"container.image.repository\":\"docker.io/library/nginx\",\"evt.time\":1675286222642052704,\"fd.name\":\"/etc/26\",\"k8s.ns.name\":\"default\",\"k8s.pod.name\":\"nginx-test2\",\"proc.aname[2]\":null,
   ...
   # print("INSIDE parseJSON the original string is: ", str(source))
   ...
   res = source.replace('\"','"')
   # printing result
   print("NB! The extracted CLEAN LOG JSON Data type, values are: ", type(res), res)
   print ("--------------------------------------------")
